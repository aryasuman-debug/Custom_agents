...
model: "GPT-5 (copilot)"
argument-hint: "Sync backlog artifacts to Jira for a project, or create/test/query Jira issues via the jira MCP server"
user-invocable: true
agents: []
---
...
- `RUN JIRA PUSH PROJECT=<project-name>`
- `RUN JIRA TEST_ISSUE PROJECT=<jira-project-key>`
- natural language equivalents such as "push the CodEval backlog to Jira"
...
## Source of Truth
Preferred inputs:
- `projects/<project-name>/artifacts/epics/epics-bundle.json`

Fallback inputs:
- `projects/<project-name>/artifacts/epics/`
...
If `storyCategory` is present, preserve it in Jira descriptions for compatibility. Do not require it for sync.
...
Category: <story.storyCategory if present>
Actor: <story.actor>
...
2. Read the canonical bundle, or fall back to canonical materialized files if the bundle is missing
...
```json
{
  ...
  "dryRun": false,
  "bundlesProcessed": [
    "projects/CodEval/artifacts/epics/epics-bundle.json"
  ],
  "epics": {
    "created": [
...
## Approach
1. Parse the user request into project and Jira target context.
